...
import 'package:racunalni_sah/horsey.dart';
...
class Skewer extends StatefulWidget {
  const Skewer({super.key});
...
class _Tactics extends State<Skewer> {
  ChessBoardController controller = ChessBoardController();
  List<String> correctMoves = ['Rh8', 'Rh7+', 'Rxa7'];
  int moveIndex = 0;

  @override
  void initState() {
    super.initState();
    controller.game.load('R7/P4k2/5p2/6p1/7p/7P/r5PK/8 w - - 0 49');
  }
                  ...
                  onMove: () {
                    ...
                    if (pgn.isNotEmpty) {
                      ...
                      if (lastMove != correctMoves[moveIndex]) {
                                        ...
                                        setState(() {
                                          if (moveIndex == 0) {
                                            controller.game.load(
                                                'R7/P4k2/5p2/6p1/7p/7P/r5PK/8 w - - 0 49');
                                          } else if (moveIndex == 1) {
                                            controller.game.load(
                                                '7R/r4k2/5p2/6p1/7p/7P/6PK/8 w - - 0 49');
                                          } else if (moveIndex == 2) {
                                            controller.game.load(
                                                '8/r6R/4kp2/6p1/7p/7P/6PK/8 w - - 0 49');
                                          }
                                        });
                                      },
                                    ),
                        ...
                        moveIndex++;
                        if (moveIndex == 1) {
                          controller.game.move(('Rxa7'));
                        } else if (moveIndex == 2) {
                          controller.game.move(('Ke6'));
                        }
                      }
                    }
                  },
